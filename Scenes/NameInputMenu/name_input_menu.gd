...
func _on_line_edit_text_submitted(new_text):
	$LineEdit.text = ""
	$LineEdit.placeholder_text = "Sumbitting..."
	
	var sw_result: Dictionary = await SilentWolf.Scores.get_scores().sw_get_scores_complete
	print("Scores: " + str(sw_result.scores))
	
	for score in sw_result.scores:
		if new_text == score.player_name:
			$LineEdit.text = ""
			$LineEdit.placeholder_text = "Taken"
			return
	
	for bad in bad_words:
		new_text = new_text.to_lower()
		if new_text.find(bad) != -1:
			$LineEdit.text = ""
			$LineEdit.placeholder_text = "Inappropriate"
			return
	
	SaveManager.save.name = new_text
	SaveManager.save_game()
	$LineEdit.placeholder_text = "Submitted!"
	$OkButton.show()
